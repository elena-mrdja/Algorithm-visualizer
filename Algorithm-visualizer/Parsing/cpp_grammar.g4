...
//our grammar
program: INT MAIN OPEN_PAREN CLOSE_PAREN block;

block: OPEN_BRACE stmts CLOSE_BRACE;

stmts: : varDec stmts| block stmts| assign stmts| assign stmts| while stmts| if else stmts| return stmts| ;

varDec: type VAR EQUAL exp SEMICOLON;

type: INT| bool| STR| CHAR| DOUBLE;

bool: TRUE| FALSE;

assign: VAR EQUAL exp SEMICOLON;

while: WHILE OPEN_PAREN exp CLOSE_PAREN block;

ifelse: IF OPEN_PAREN exp CLOSE_PAREN block ifrest;

ifrest: ELSE block| ELSE ifelse| ;

return: RETURN exp SEMICOLON;

exp: VAR| NUM| TRUE| FALSE| OPEN_PAREN exp CLOSE_PAREN| exp binop exp| unop exp| exp unop;
