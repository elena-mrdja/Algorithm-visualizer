...
statement: exp SEMICOLON | varDec | assign | ifelse | whileStmt | returnStmt | mainStmt;

mainStmt: type MAIN LP RP block;

assign: VAR identifier EQ val=exp SEMICOLON;

exp: VAR| INTEGER| boolType| string | LP exp RP| exp binOp exp| unop exp| exp unop | unop | identifier;

varDec: type identifier EQ val=exp SEMICOLON | type VAR;
...
//good below
block: LCB stmts* RCB;
stmts: varDec stmts| assign stmts| whileStmt stmts| ifelse stmts| returnStmt stmts| exp SEMICOLON;
...
doubleType: INTEGER DOT INTEGER;

string: QUOTE content=.*? QUOTE;

type: INT| STR| CHAR| DOUBLE;
